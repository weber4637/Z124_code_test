  ...
  )
{
  ...
  BOOLEAN               DeviceHiden; // Denverton AptioV override - EIP#288063
...
// Denverton AptioV override Start - EIP#288063
    //
    //Unhide device if hidden
    //
    DeviceHiden = PciRead32(PciLibAddress + R_HSUART_RDCFG) & B_HSUART_RDCFG_HIDE? TRUE:FALSE;
    if (DeviceHiden) {
      PciWrite32(PciLibAddress + R_HSUART_RDCFG, 0);
    }
// Denverton AptioV override End- EIP#288063
    //
...
// Denverton AptioV override Start - EIP#288063
      if (DeviceHiden) {
        PciWrite32(PciLibAddress + R_HSUART_RDCFG, B_HSUART_RDCFG_HIDE);
      }
// Denverton AptioV override End- EIP#288063
      return 0;
    }
  ...
  )
{
  ...
  UINTN  FifoSize;
  //*** AMI PORTING BEGIN ***//
  // Improvement: Serial Port Timeout.
  // Original implementation waits indefinitely for serial port status bits, which leads
  // to system lock up if serial port is missing or not properly enabled.
  // This and other modifications in this functions are to add timeout support.
  UINT32 i = 0;
  //*** AMI PORTING END *****//
