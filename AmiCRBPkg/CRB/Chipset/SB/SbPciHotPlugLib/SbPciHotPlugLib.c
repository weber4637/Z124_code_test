...
/** @file SbPciHotPlugLib.c
    Library Class for AMI PCI Hot Plug Support Functions. 
...
#include <Setup.h>                //Denverton AptioV Override - EIP#348922
...
BOOLEAN HpcCheckHpCompatibleHost(PCI_HOST_BRG_DATA *HostBrg){
    ...
    if(HostBrg->HpcData == NULL)
        PCI_TRACE((TRACE_PCI,"[DNV HP]: HostBrg->HpcData is NULL\n"));
    
    if(HostBrg->HpcData->HpcLocCount > 0){
        PCI_TRACE((TRACE_PCI,"[DNV HP]: HpcLocCount is %d\n", HostBrg->HpcData->HpcLocCount));
        return TRUE;
    }
    
    PCI_TRACE((TRACE_PCI,"[DNV HP]: HpcCheckHpCompatibleHost function return FALSE\n"));
	return FALSE;
}
...
//-------------------------------------
    ...
    PCI_TRACE((TRACE_PCI,"[DNV HP]: Update get Padding Setup Data in HpcUpdateLocationData function\n"));
	
	PCI_TRACE((TRACE_PCI,"[DNV HP]:  parent->Bus : %x  ; pcidp.Device = %x   \n", parent->Bus, pcidp.Device));
    ...
    if (parent->PciDevFlags.Bits.FixedBus){
        if ((parent->Bus == 0 && pcidp.Device == 0x09) ||
            (parent->Bus == 0 && pcidp.Device == 0x0A) ||
            (parent->Bus == 0 && pcidp.Device == 0x0B) ||
            (parent->Bus == 0 && pcidp.Device == 0x0C) ||
            (parent->Bus == 0 && pcidp.Device == 0x0E) ||
            (parent->Bus == 0 && pcidp.Device == 0x0F) ||
            (parent->Bus == 0 && pcidp.Device == 0x10) ||
            (parent->Bus == 0 && pcidp.Device == 0x11)) {
			
			idx = (pcidp.Device) - (0x09);
			if (pcidp.Device > 0x0C){
			   idx -= 1 ;
			   }
		
            locdata->HpSetup.ArrayField.BusPadd      = mSystemConfiguration.PcieExtraBusRsvd[idx];
            locdata->HpSetup.ArrayField.IoPadd       = mSystemConfiguration.PcieIoRsvd[idx] * 0x400;
			locdata->HpSetup.ArrayField.Mmio32Padd = mSystemConfiguration.PcieMemRsvd[idx] * 0x100000;
	        ...
	        if (!EFI_ERROR(Status) && SetupData.Above4gDecode){
			   locdata->HpSetup.ArrayField.Mmio64PfPadd = mSystemConfiguration.PciePFMemRsvd[idx] * 0x100000;
            }
			else{
			     locdata->HpSetup.ArrayField.Mmio32PfPadd   = mSystemConfiguration.PciePFMemRsvd[idx] * 0x100000;

			}
            //Denverton AptioV Override End- EIP#348922

            PCI_TRACE((TRACE_PCI,"Setup Data: Rsvd Bus:%x, Rsvd Mem:%x, Rsvd FMem:%x, Rsvd Io:%x\n", mSystemConfiguration.PcieExtraBusRsvd[idx], mSystemConfiguration.PcieMemRsvd[idx], mSystemConfiguration.PciePFMemRsvd[idx], mSystemConfiguration.PcieIoRsvd[idx]));
          }
...
//-------------------------
    ...
    for(i=0,hpbcnt=0; i<gSdlPciData->PciDevCount; i++){
    	...
    	if(sdltmpParent->PciDevFlags.Bits.RootBridgeType && sdltmpParent->PciDevFlags.Bits.FixedBus){
    	    PCI_TRACE((TRACE_PCI,"[DNV HP]: Parent = %s in SDL\n", sdltmpParent->AslName));
        if ((sdltmpParent->Bus == 0 && sdltmp->Device == 0x09) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x0A) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x0B) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x0C) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x0E) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x0F) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x10) ||
            (sdltmpParent->Bus == 0 && sdltmp->Device == 0x11)) {
			idx = (sdltmp->Device) - (0x09);
			if (sdltmp->Device > 0x0C){
			   idx -= 1 ;
			   }

				PCI_TRACE((TRACE_PCI,"[DNV HP]: mSystemConfiguration.PcieRootPortHPE[idx] =  %x\n", mSystemConfiguration.PcieRootPortHPE[idx]));

				sdltmp->PciDevFlags.Bits.HotPlug = mSystemConfiguration.PcieRootPortHPE[idx];

				PCI_TRACE((TRACE_PCI,"[DNV HP]: sdltmp->PciDevFlags.Bits.HotPlug  =  %x\n", sdltmp->PciDevFlags.Bits.HotPlug));
    	    }    
    	}
    	

		PCI_TRACE((TRACE_PCI,"[DNV HP]: sdltmp->PciDevFlags.Bits.HotPlug =  %x\n", sdltmp->PciDevFlags.Bits.HotPlug));
		PCI_TRACE((TRACE_PCI,"[DNV HP]: sdltmp->PciDevFlags.Bits.Pci2PciBridgeType =  %x\n", sdltmp->PciDevFlags.Bits.Pci2PciBridgeType));
    		...
    		if(AmiSdlCheckIfParent(i, RootBrg->RbSdlIndex)){
    			...
    			for(j=0,br=TRUE,sltcnt=0; j<hpscnt; j++){
    				...
    				if(sdltmp->PciDevFlags.Bits.OnBoard){
    					...
    					PCI_TRACE((TRACE_PCI,"\tFound Slot# 0x%X(%d), SdlIndex 0x%X(%d) @ B:%x|D:%x|F:%x; HpDisable=%d; ",
...
//------------------------
    ...
    PCI_TRACE((TRACE_PCI,"[DNV HP]: HpcGetResourcePadding Entry !!!\n"));
    ...
    PCI_TRACE((TRACE_PCI,"[DNV HP]: Hotplug location: %x -> B:%x|D:%x|F:%x\n", HpcPciAddress, Bus, Dev, Fun));
    ...
    for(i=0, qw=pd; i<ptMaxType; i++){
        ...
        if(locdata->HpSetup.ARRAY[i]!=0){
          if ((Bus == 0 && Dev == 0x09) ||
             ...
             (Bus == 0 && Dev == 0x0F) ||
             (Bus == 0 && Dev == 0x10) ||
             (Bus == 0 && Dev == 0x11)) { // SB PCIE root port
             ...
             switch(i){
                    case ptMmio32:
                    case ptMmio64:
                         HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], mSystemConfiguration.PcieMemRsvdalig[idx]);
                        break;
                    case ptMmio32pf:
                    case ptMmio64pf:
                        HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], mSystemConfiguration.PciePFMemRsvdalig[idx]);
                        break;
                    default:
            ...
            PCI_TRACE((TRACE_PCI,"[DNV HP]: resource type: %x. resource size: %x\n", i, locdata->HpSetup.ARRAY[i]));
            qw++;
