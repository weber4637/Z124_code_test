...
*/
  DEBUG ((AmiCryptoPkg_DEBUG_LEVEL, "Find Key Ffs %r addr=%X (%X,%X), size=%d\n", Status, (UINT32)*Buffer, ((UINT8*)*Buffer)[0], ((UINT8*)*Buffer)[1], *Size));
...
//    UINT8                   Byte;
    ...
    // DEBUG ((AmiCryptoPkg_DEBUG_LEVEL, "*PeiServices == *ppPS(ppPS) %x == %x(%x)\n",  *PeiServices, *ppPS, ppPS ));
...
/*
            ...
            if(i == Size) {
                DEBUG ((AmiCryptoPkg_DEBUG_LEVEL, "Key FFS with dummy Key detected...\n"));
                break;
            }
*/            
