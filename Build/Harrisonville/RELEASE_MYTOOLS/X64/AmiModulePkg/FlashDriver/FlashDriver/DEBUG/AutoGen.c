...
GLOBAL_REMOVE_IF_UNREFERENCED GUID gEfiCallerIdGuid = {0xCBC59C4A, 0x383A, 0x41eb, {0xA8, 0xEE, 0x44, 0x98, 0xAE, 0xA5, 0x67, 0xE4}};

GLOBAL_REMOVE_IF_UNREFERENCED CHAR8 *gEfiCallerBaseName = "FlashDriver";
...
GLOBAL_REMOVE_IF_UNREFERENCED const UINT32 _gPcd_FixedAtBuild_AmiPcdFlashPropertyMask = _PCD_VALUE_AmiPcdFlashPropertyMask;
...
EFI_STATUS
EFIAPI
RuntimeAmiRomLayoutLibConstructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  );
  ...
  )
{
  ...
  Status = RuntimeAmiFlashLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = RuntimeAmiRomLayoutLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
...
EFI_STATUS
EFIAPI
RuntimeAmiRomLayoutLibDestructor (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  );
  ...
  )
{
  ...
  Status = RuntimeAmiRomLayoutLibDestructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = RuntimeAmiFlashLibDestructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
...
{
  return FlashDriverInit (ImageHandle, SystemTable);
}
