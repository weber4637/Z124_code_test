...
GLOBAL_REMOVE_IF_UNREFERENCED GUID gEfiCallerIdGuid = {0x0C375A90, 0x4C4C, 0x4428, {0x8E, 0xA0, 0x53, 0x1B, 0xE8, 0x95, 0x9B, 0xF7}};

GLOBAL_REMOVE_IF_UNREFERENCED CHAR8 *gEfiCallerBaseName = "FlashDriverSmm";
  ...
  )
{
  ...
  Status = DxeAmiFlashLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = SmmAmiRomLayoutLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  if (SetJump (&mJumpContext) == 0) {
    ExitDriver (FlashDriverSmmInit (ImageHandle, SystemTable));
    ASSERT (FALSE);
  }
