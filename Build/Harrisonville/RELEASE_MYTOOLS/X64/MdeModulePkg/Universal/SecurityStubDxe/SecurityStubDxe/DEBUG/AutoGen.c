...
GLOBAL_REMOVE_IF_UNREFERENCED EFI_GUID gAmiBdsConnectPolicyProtocolGuid = { 0x38d65ec3, 0x8f39, 0x4660, { 0xb8, 0xa6, 0xf3, 0x6a, 0xa3, 0x92, 0x54, 0x75 } };
...
#define _PCD_VALUE_AmiPcdLoadUnsignedEmbeddedApplications  0U
GLOBAL_REMOVE_IF_UNREFERENCED const BOOLEAN _gPcd_FixedAtBuild_AmiPcdLoadUnsignedEmbeddedApplications = _PCD_VALUE_AmiPcdLoadUnsignedEmbeddedApplications;
  ...
  )
{
  ...
  Status = AmiImageTypeLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = AmiConnectPolicyLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = DxeTpmMeasureBootLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  Status = DxeTpmMeasureBootLibDestructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
