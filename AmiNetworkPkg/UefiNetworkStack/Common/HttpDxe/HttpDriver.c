...
EFI_STATUS
HttpCreateService (
  IN  EFI_HANDLE            Controller,			// AMI PORTING
  ...
  )
{
  ...
  *ServiceData = NULL;
  ...
  HttpService->ControllerHandle = Controller;		// AMI PORTING
  ...
  if (!UsingIpv6) {
    if (HttpService->Tcp4ChildHandle != NULL) {
      gBS->CloseProtocol (
             ...
             HttpService->Ip4DriverBindingHandle,			// AMI PORTING
    ...
    if (HttpService->Tcp6ChildHandle != NULL) {
      gBS->CloseProtocol (
             ...
             HttpService->Ip6DriverBindingHandle,			// AMI PORTING
             HttpService->ControllerHandle
             );
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Status = HttpCreateService (ControllerHandle, &HttpService);			// AMI PORTING
    if (EFI_ERROR (Status)) {
      return Status;
    }
  ...
  if (IpVersion == IP_VERSION_4) {
    HttpService->Ip4DriverBindingHandle = This->DriverBindingHandle;			// AMI PORTING
    ...
    UsingIpv6 = TRUE;
    HttpService->Ip6DriverBindingHandle = This->DriverBindingHandle;			// AMI PORTING
