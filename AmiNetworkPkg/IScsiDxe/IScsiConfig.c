...
extern BOOLEAN gIsInitiatorNameForceSet;
extern CHAR8	gForcedInitiatorName[ISCSI_NAME_MAX_SIZE];
...
EFI_STATUS
DefaultIscsiInitiatorNameHook(OUT CHAR8 *Buffer)
{
...
#endif	// ENABLE_CHAP_SECRET_ENCRYPTION
// AMI PORTING ENDS
  ...
  )
{
  ...
  if (AttemptConfigOrder != NULL) {
    ...
    NET_LIST_FOR_EACH (Entry, &mPrivate->AttemptConfigs) {
      ...
      // AMI Porting End: Deleting the Attempt from DynamicOrderedList as well
  ...
  )
{
  ...
  *Progress = Request;
  ...
  if (EFI_ERROR (Status)) {
    ...
    IfrNvData->IsInitiatorNameValid = FALSE;	// AMI PORTING: Update flag for invalid Initiator name as FALSE.
  } else {
    AsciiStrToUnicodeStrS (
      InitiatorName,
    ...
    IfrNvData->IsInitiatorNameValid = TRUE;		// AMI PORTING : Update flag for Valid Initiator name as TRUE.
  }
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGING) {
    ...
    // AMI Porting End: Taking Backup of both DynamicOrderedList and DeleteAttempts List
...
// AMI PORTING ENDS - RSA iSCSI Support.
  ...
  //
        ...
        if ( !EFI_ERROR (gIScsiInitiatorName.Get (&gIScsiInitiatorName, &BufferSize, IScsiName)) ) {
          AsciiStrToUnicodeStrS (IScsiName, IfrNvData->InitiatorName, sizeof(IfrNvData->InitiatorName) / sizeof(IfrNvData->InitiatorName[0]));
        }
      }
      //
    ...
    // AMI Porting End: Restoring both DynamicOrderedList and DeleteAttempts List 
      *ActionRequest = EFI_BROWSER_ACTION_REQUEST_FORM_DISCARD_EXIT;
      break;
    ...
    case KEY_TARGET_NAME:
      ...
      if (EFI_ERROR (Status)) {
        ...
        AsciiStrToUnicodeStrS (Private->Current->SessionConfigData.TargetName, IfrNvData->TargetName, sizeof(IfrNvData->TargetName)/sizeof(IfrNvData->TargetName[0]));
        //
        // Set Status as EFI_SUCCESS to update IfrNvData in Form browser data
        //
        Status = EFI_SUCCESS;
