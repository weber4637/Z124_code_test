...
#include "IScsiImpl.h"
#include "Token.h"
#include "AmiLib.h"// AMI PORTING - Initiator IP is displayed if attempt is created for connection-less NIC.
...
//
BOOLEAN   SameNicIndex;       // AMI PORTING.

//
// AMI PORTING STARTS - Initiator IP is displayed if attempt is created for connection-less NIC.
//
#if (NET_PKG_AMI_PORTING_ENABLE)
/**
  Compares two MACs and returns the BOOLEAN result.

  @param[in]        ConfigData       The current attempt configuration data.
  @param[in]        Controller       The handle of the current NIC.

  @retval TRUE		If both the MACs are same.
  @retval FALSE		If the MACs are different.

**/
BOOLEAN IScsiAttemptCheckMatchingController(ISCSI_ATTEMPT_CONFIG_NVDATA *ConfigData, EFI_HANDLE Controller)
{
  EFI_MAC_ADDRESS           MacAddress;
  UINTN                     HwAddressSize;
  // Variables used for conversions.
  CHAR16					MacString16[20] = {0};
  CHAR16					ConfigMacString16[ISCSI_MAX_MAC_STRING_LEN] = {0}; // AMI PORTING
  
  EFI_STATUS 				Status;
  BOOLEAN 					IsMacSame = FALSE;
  
  if((ConfigData == NULL) || (Controller == NULL))
	  return FALSE;

  // Get the MAC address from the controller handle passed.
  Status = NetLibGetMacAddress (Controller, &MacAddress, &HwAddressSize);
  if(!EFI_ERROR(Status))
    {
  	  IScsiMacAddrToStr(&MacAddress,(UINT32)HwAddressSize,0,MacString16);
  	  if(MacString16[0] != '\0')
  	  {
  		  // Convert CHAR8 to CHAR16
  		  AsciiStrToUnicodeStrS(ConfigData->MacString, ConfigMacString16, sizeof(ConfigMacString16)/sizeof(ConfigMacString16[0]));
  		  if(ConfigMacString16[0] != '\0')
  		  {
  			  if(StrStr(ConfigMacString16,MacString16) != 0)
  			  {
  			      IsMacSame = TRUE;
  			  }
  		  }
...
//
// AMI PORTING ENDS - Initiator IP is displayed if attempt is created for connection-less NIC.
//
  ...
  )
{
  ...
  ZeroMem((UINT8 *)Ip, sizeof(EFI_IP_ADDRESS)); // AMI PORTING - Clearing Ip address to avoid junk when changed from ip4 to ip6
  ...
  )
{
  ...
  UINT16                      VlanId;
   ...
   //
   // Set default value as FALSE for every function call.
   //
   SameNicIndex = FALSE;          // AMI PORTING.
  ...
  NET_LIST_FOR_EACH (Entry, &mPrivate->NicInfoList) {
    ...
    if (NicInfo->HwAddressSize == HwAddressSize &&
      ...
      //
      // If NIC info already exists, set "SameNicIndex" as true.
      //
      SameNicIndex = TRUE;                // AMI PORTING.
      return EFI_SUCCESS;
    }
  ...
  )
{
  ...
  EFI_STATUS 		      	  RsaStatus = EFI_NOT_FOUND; //  AMI PORTING - RSA iSCSI Support.

  //
  // AMI PORTING START.
  //
  //
  // Save MpioCount and SinglePathCount in local variables as they would be used later.
  //
...
// AMI PORTING STARTS - RSA iSCSI Support.
  ...
  // Initialize RSA configuration Data
  RsaStatus = RSAInitializeBMCInformation();
  DEBUG((DEBUG_INFO,"\n RSAISCSI: RSAInitializeBMCInformation Status :: %r \n",RsaStatus));
  // Check RSA BMC Data availability
  // Get AttemptConfigOrder
  if(RSAIsBMCDataAvailable())
  {
	 // Use RSA interface to get the AttemptConfigOrder
	  AttemptConfigOrderSize = 0;
	  RsaStatus = RSAGetIScsiAttemptConfigOrder(&AttemptConfigOrder, (UINT32*)&AttemptConfigOrderSize);
  }
  else
...
// AMI PORTING STARTS - RSA iSCSI Support.
  ...
  if(RSAIsBMCDataAvailable())
  {
	  // Use RSA interface to get the InitiatorName
	  Status = RSAGetIScsiInitiatorName(mPrivate->InitiatorName);
  }
  else
#endif	// RSA_ISCSI_SUPPORT
// AMI PORTING ENDS - RSA iSCSI Support.
  ...
  for (Index = 0; Index < AttemptConfigOrderSize / sizeof (UINT8); Index++) {
    ...
    // AMI PORTING STARTS - Initiator IP is displayed if attempt is created for connection-less NIC.
#if (NET_PKG_AMI_PORTING_ENABLE)
    else if (AttemptTmp != NULL && !IScsiAttemptCheckMatchingController(AttemptTmp,Private->Controller)) {
    	continue;
    }
#endif	// NET_PKG_AMI_PORTING_ENABLE
    // AMI PORTING ENDS - Initiator IP is displayed if attempt is created for connection-less NIC.
...
// AMI PORTING STARTS - RSA iSCSI Support.
          ...
          {
              RSAUpdateBMCIScsiAttemptDataCache(mPrivate->PortString, AttemptTmp);
          }
#endif	// RSA_ISCSI_SUPPORT
// AMI PORTING ENDS - RSA iSCSI Support.
// AMI PORTING STARTS          
...
// AMI PORTING STARTS - RSA iSCSI Support.
#if (RSA_ISCSI_SUPPORT)
	// If BMC data is available, get it.
    if(RSAIsBMCDataAvailable())
    {
        RSAGetBMCIScsiAttemptData (mPrivate->PortString, &AttemptConfigData);
    }
    else
#endif	// RSA_ISCSI_SUPPORT
// AMI PORTING ENDS - RSA iSCSI Support.
...
// AMI PORTING STARTS
    ...
    {
    	ZeroMem(&AttemptConfigData->SessionConfigData.LocalIp,sizeof(EFI_IP_ADDRESS));	// AMI PORTING - Initiator IP is displayed if attempt is created for connection-less NIC.
    }
#endif
// AMI PORTING ENDS
    ...
    //
        ...
        if (AttemptConfigData->SessionConfigData.Enabled != ISCSI_DISABLED &&
            ...
            // AMI PORTING - The following extra condition check has been added so that 
            // DHCP will happen only if the controller MAC and attempt MAC are same.
            IScsiAttemptCheckMatchingController(AttemptConfigData,Private->Controller)) {
#else
            if (AttemptConfigData->SessionConfigData.Enabled != ISCSI_DISABLED &&
                AttemptConfigData->SessionConfigData.InitiatorInfoFromDhcp) {
#endif
...
// AMI PORTING ENDS
    ...
    //
    //  AMI PORTING START - Release the string pointer which is pointed in if or else if blocks.
    // 
    if(CharPtr != NULL)	{   
       FreePool (CharPtr);
       CharPtr = NULL;
    ...
    //
    //  AMI PORTING END - Release the string pointer which is pointed in if or else if blocks.
    //
