...
1. Use Aptio FW image 'AMIROM.fd' created on
    06/30/2016 at 10:42:26 and including:
    ...
    b)FwCapsule Hdr ffs populated with Capsule signing parameters:
      -m -r RomLayoutEx.bin -c priKey -k priKey -q

2. Provide 2048bit RSA Key file in PKCS#1v2.1 DER (PEM)format.
    Root Key Certificate key (full RSA key)  - priKey
    Signing Certificate key (full RSA key) - priKey
    Signing Certificate key (public key part) - pubKey

=============================================================
=== Run Cryptocon.exe script to generate Signed FwCapsule
=============================================================

 CryptoCon -@ -y -n -c priKey -k priKey -q -f file_in -o file_out
