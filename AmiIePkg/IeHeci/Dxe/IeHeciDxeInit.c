...
@brief:
 ...
 IE-HECI DXE Driver, implements IE-HECI Protocol.
...
IE_HECI1_PROTOCOL_INSTANCE  *gIeProtocolInstance;
...
/**
  The entry point of the HECI DXE Driver. Installs HECI Protocol interface.
...
)
{
  ...
  gIeProtocolInstance = AllocateZeroPool(sizeof (IE_HECI1_PROTOCOL_INSTANCE));
  if (!gIeProtocolInstance) {
     return EFI_OUT_OF_RESOURCES;
  }

  gIeProtocolInstance->Protocol.InitializeHeci = HeciInitialize;
  gIeProtocolInstance->Protocol.ReadMsg = HeciReadMsg;
  gIeProtocolInstance->Protocol.SendMsg = HeciSendMsg;
  gIeProtocolInstance->Protocol.SendwAck = HeciSendwAck;
  gIeProtocolInstance->Heci.Bus = IE_HECI1_BUS;
  gIeProtocolInstance->Heci.Dev = IE_HECI1_DEV;
  gIeProtocolInstance->Heci.Fun = IE_HECI1_FUN;
  gIeProtocolInstance->Heci.Hidm = HECI_HIDM_MSI;
  gIeProtocolInstance->Heci.Mbar = HECI1_IE_MBAR_DEFAULT;

  Status = gBS->InstallProtocolInterface (
                        &IeHandle,
                        &gDxeIeHeciProtocolGuid,
                        EFI_NATIVE_INTERFACE,
                        &gIeProtocolInstance->Protocol);
  if (EFI_ERROR(Status)) {
     FreePool(gIeProtocolInstance);
     return Status;
  }
...
/**
  Initialize HECI-1 for use in DXE phase.
