...
BOOLEAN InitMicrocodeVariables()
{
    ...
    pFit = *(FIT_ENTRY**)(UINTN)0xffffffc0;
    if ((UINT32)(UINTN)pFit == 0xffffffff || (UINT32)(UINTN)pFit == 0xeeeeeeee || *(UINT32*)(UINTN)pFit != SIGNATURE_32 ('_', 'F', 'I', 'T')) pFit = NULL;
...
VOID PresenceTest(SMI_UC_DWORD_REGS *Regs)
{
    ...
    Regs->EBX = ('I' << 24) | ('N' << 16) | ('T' << 8) | 'E';  //INTE
    Regs->EBX = ('L' << 24) | ('P' << 16) | ('E' << 8) | 'P';  //LPEP
...
VOID ReadUpdateData(SMI_UC_DWORD_REGS *Regs)
{
    ...
    Status = CheckAddressRange (UpdateBuffer, MICROCODE_SIZE(Header));
    if(EFI_ERROR(Status)) return;
...

{
    ...
    Status = gSmst->SmmLocateProtocol(&	gEfiSmmCpuProtocolGuid, NULL, (VOID**)&SmmCpuProtocol);
	ASSERT_EFI_ERROR(Status);
