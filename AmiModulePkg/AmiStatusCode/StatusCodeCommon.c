...
static EFI_GUID gStatusCodeDataTypeAssertGuid = EFI_STATUS_CODE_DATA_TYPE_ASSERT_GUID;
...
)
{
    ...
    if (Data != NULL)
    {
        ...
        if (!guidcmp(&(Data->Type), &gEfiStatusCodeDataTypeStringGuid))
        {
        ...
        else if (!guidcmp(&(Data->Type), &gEfiStatusCodeSpecificDataGuid))
        {
        ...
        else if ( !guidcmp (&Data->Type, &gStatusCodeDataTypeAssertGuid) )
        {
...
extern DEBUG_SERVICE_PRINT_GATE_FUNCTION* DebugServicePrintGate;

extern DEBUG_SERVICE_PRINT_FUNCTION* DebugServicePrintList[];

extern DEBUG_SERVICE_ASSERT_GATE_FUNCTION* DebugServiceAssertGate;

extern DEBUG_SERVICE_ASSERT_FUNCTION* DebugServiceAssertList[];
...
EFI_STATUS DebugServicePrint(IN CHAR8 *String, IN UINTN ErrorLevel)
{
    UINTN i;
    
    for (i=0; DebugServicePrintList[i]; i++) DebugServicePrintList[i](String, ErrorLevel);
    
    return EFI_SUCCESS;
}

EFI_STATUS DebugServiceAssert(IN UINT8 *String, IN CONST CHAR8  *FileName, IN UINTN LineNumber)
{
    UINTN i;
    
    for (i=0; DebugServiceAssertList[i]; i++) DebugServiceAssertList[i](String, FileName, LineNumber);
...
BOOLEAN NullDebugServicePrintGate(IN CHAR8 *String, IN UINTN ErrorLevel)
{
    return TRUE;
}

BOOLEAN NullDebugServiceAssertGate(IN CONST CHAR8 *FileName, IN UINTN LineNumber)
{
    return TRUE;
}
...
{
    BOOLEAN DebugServicePrintEnable;
    
    if (Format==NULL) return;
    AsciiVSPrint ((CHAR8*)String, StringMaxSize, Format, VaListMarker);

    DebugServicePrintEnable = DebugServicePrintGate(String, ErrorLevel);
    
    if (DebugServicePrintEnable == FALSE) return;

    DebugServicePrint(String, ErrorLevel);
...
{
  	CHAR8 Buffer[2] = {0};
  	BOOLEAN ResumableAssert = FALSE;
	BOOLEAN DebugServiceAssertEnable;
  	
	DebugServiceAssertEnable = DebugServiceAssertGate(FileName, LineNumber);
    
    if (DebugServiceAssertEnable == FALSE) return;
  	// To enable resumable assert DEBUG_PROPERTY_ASSERT_BREAKPOINT_ENABLED and DEBUG_PROPERTY_ASSERT_DEADLOOP_ENABLED bits of 
  	// PcdDebugPropertyMask must be set to 0
  	if (((PcdGet8(PcdDebugPropertyMask) & (DEBUG_PROPERTY_ASSERT_BREAKPOINT_ENABLED | DEBUG_PROPERTY_ASSERT_DEADLOOP_ENABLED)) == 0))
  	{
  		ResumableAssert = TRUE;
  		AsciiSPrint ((CHAR8*)String, StringMaxSize, "ASSERT %a(%d): %a\n Press any key to continue. \n", FileName, LineNumber, Description);
  	}
  	else 
  		AsciiSPrint ((CHAR8*)String, StringMaxSize, "ASSERT %a(%d): %a\n", FileName, LineNumber, Description);
  	
    DebugServiceAssert(String, FileName, LineNumber);
    StringStatusReport(PeiServices, String);
    
    while (ResumableAssert)
  	{
