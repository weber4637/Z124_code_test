...
//AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"VerifyVariable CertHdr Size 0x%x (expected(0x230)\n",AMI_AMI_AUTHINFO_SIZE(CertData)));
    ...
    if ((CertData->AuthInfo.Hdr.wCertificateType != WIN_CERT_TYPE_EFI_GUID) ||  
        ...
        //
AVAR_TRACE((EFI_D_ERROR,"VerifyVariable AuthHdr GUID test failed!\nWinCert_Type:\nExpected  %x\nReceived %x\nGUID:\nExpected  %g\nReceived %g\n",
...
// SCT 2.3.1 TW Plugfest uses wrong GUID
    ...
    {
        AVAR_TRACE((EFI_D_ERROR,"VerifyVariable DataSize test failed!\nDataSize(%x) < AuthHdrSize (%x)\n", *DataSize, CertHdrSize));
        return EFI_SECURITY_VIOLATION;
    }
    //
    ...
    //
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"Check MC:\nOld=%x\nNew=%x\n",ExtFlags->Mc, CertData->MonotonicCount));
...
AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"Upd=%x\n", ExtFlags->Mc));
    ...
    if (OldData && MemCmp(&ExtFlags->KeyHash[0], PubKeyHash, HASH_SHA256_LEN)){
        AVAR_TRACE((EFI_D_ERROR,"Self Signed MC Var Key Compare FAILED!\n"));
        return EFI_SECURITY_VIOLATION;
    }
