...
//    AVarRuntime = TRUE;
    ...
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"SMM.mSecureBootMode %X\n", mSecureBootMode));
    //
    ...
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"SMM.mVendorKeyState %X\n", mVendorKeyState));
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"SecureBootMode Update to %x\n", NewMode));
    mSecureBootMode = NewMode;
  } else {
    AVAR_TRACE((SecureBoot_DEBUG_LEVEL, "Update SecureBootMode Variable fail %r\n", Status));
  }
  ...
  )
{
  ...
  AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"Transfer SecureBootMode %x ==> to SecureBootMode %x\n", CurMode, NewMode));
...
AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"SigList.Type-"));
        if(Index >= SIGSUPPORT_NUM)
           return EFI_SECURITY_VIOLATION; // Sig not found

AVAR_TRACE((SecureBoot_DEBUG_LEVEL,"OK\nSigList.Size-"));
...
//AVAR_TRACE((SecureBoot_DEBUG_LEVEL, "Exit - %r, Data - %X, Size %d, Attributes %x\n",Status, *Data, *DataSize, *Attributes));
        return Status;
    }
