...
// AMI Test KEK (TBD)
    ...
    while(SecureVariableFileName[Index] != NULL)
    {
        ...
        DEBUG((SecureBoot_DEBUG_LEVEL,"NV Var %s(%d),  status=%r\n",  SecureVariableFileName[Index], Size, Status));
...
//#if !defined( TSE_FOR_APTIO_4_50)
                Status = ValidateSignatureList (Buf1, Size, &SigCount);
                if(EFI_ERROR(Status)) {
...
//#endif
                ...
                DEBUG((SecureBoot_DEBUG_LEVEL,"RT Var %s(%d),  status=%r\n",  SecureVariableFileName[Index], Size2, Status));
...
DEBUG((SecureBoot_DEBUG_LEVEL,"SigList.Type(%g) -", &(SigList->SignatureType)));
...
DEBUG((SecureBoot_DEBUG_LEVEL,"Sig.Count(%d)\n", count));
        DataSize -= SigList->SignatureListSize;
        SigList = (EFI_SIGNATURE_LIST *) ((UINT8 *) SigList + SigList->SignatureListSize);
        DEBUG((SecureBoot_DEBUG_LEVEL,"DataSize left(%X)\n", DataSize));
...
*/        
        if(InstallVars & (SET_NV_DEFAULT_KEYS | SET_RT_DEFAULT_KEYS)) {
            ...
            DEBUG((SecureBoot_DEBUG_LEVEL,"Get Raw image %s, Status %r, sz = %x\n",SecureVariableFileName[Index], Status, FileSize));
            if(EFI_ERROR(Status)) {
                Index++;
                continue;
            }
        ...
        // try to erase. should succeed if system in pre-boot and Admin mode
            ...
            DEBUG((SecureBoot_DEBUG_LEVEL,"Clear NV Var %s, Status %r\n",SecureVariableFileName[Index], Status));
        ...
        if((InstallVars & (SET_NV_DEFAULT_KEYS | SET_RT_DEFAULT_KEYS)) &&
            ...
            if(InstallVars & (SET_RT_DEFAULT_KEYS)) {
                ...
                DEBUG((SecureBoot_DEBUG_LEVEL,"Set RT Var %s, Status %r\n",SecureVariableFileNameDefault[Index], Status));
            }
            if(InstallVars & (SET_NV_DEFAULT_KEYS)) {
                ...
                if(EFI_ERROR(Status) && Status == EFI_NOT_FOUND ) {
                    ...
                    DEBUG((SecureBoot_DEBUG_LEVEL,"Set NV Var %s, Status %r\n",SecureVariableFileName[Index], Status));
                }
            }
...
//DEBUG((SecureBoot_DEBUG_LEVEL,"%r, StrRef '%s', Size %d, Token=%d\n",Status, ppData, DataSize, Token));
}
...
//----------------------------------------------------------------------
//<AMI_PHDR_END>
static EFI_STATUS FillAuthVarHdr(
    UINT8 *pVar,
    UINT8 *pCert,
    UINTN CertSize,
    UINT8 VarSetMode
...
/*      if(mAmiSig == NULL)
            Status = pBS->LocateProtocol(&gAmiDigitalSignatureProtocolGuid, NULL, &mAmiSig); 
        DEBUG((SecureBoot_DEBUG_LEVEL, "mAmiSig %x,  CertLen %x\n", mAmiSig, CertSize));
        if(mAmiSig == NULL)
            return;
...
*/
        ...
        DEBUG((SecureBoot_DEBUG_LEVEL,"Parse X.509 cert %x (sz=%d)\n", x509SignCert, CertSize));
...
DEBUG((SecureBoot_DEBUG_LEVEL,"FindInDB(x)\nDataSize In %d (0x%X)\n",*DataSize,*DataSize));
        //
...
//DEBUG((SecureBoot_DEBUG_LEVEL,"Upd SignatureListSize=%x, DataSize=%x\n",SigListNew->SignatureListSize, *DataSize));
                                    ...
                                    {  
//DEBUG((SecureBoot_DEBUG_LEVEL,"SigList is Empty!\n"));
                                        break;
                                    }         
...
DEBUG((SecureBoot_DEBUG_LEVEL,"DataSize Out: %d (0x%X)\n",*DataSize, *DataSize));
    if(*DataSize==0)
        return EFI_ALREADY_STARTED;

DEBUG((SecureBoot_DEBUG_LEVEL,"UPDATE SUCCESS!\n"));
    }
...
EnrollCertificateToDbFromEfiImageCallback(EFI_HII_HANDLE HiiHandle, UINT16 Class, UINT16 SubClass, UINT16 Key)
{
    ...
    if(!EFI_ERROR(Status) && FileBuf)
    {
        ...
        // Append AuthHdr to Var data.
        if(EFI_ERROR(FillAuthVarHdr(Data,FileBuf,size, SET_SECURE_VARIABLE_APPEND))) {
            pBS->FreePool(Data);
            goto exit;
        }
...
//        MenuSize = 2;
        ...
        if(!EFI_ERROR(Status) && FileBuf)
        {
            ...
            if(!EFI_ERROR(Status))
            {
                ...
                // Append AuthHdr to Var data.
                if(CertSel==0) {
                    if(EFI_ERROR(FillAuthVarHdr(Data,FileBuf,size,VarSetMode))) {
                        pBS->FreePool(Data);
                        goto exit;
                    }
                }
