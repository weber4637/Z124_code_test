...
{
    //DEBUG((EFI_D_INFO,"AtadSmiHandler1\n"));
    if (gAtadBuffer == NULL)
        return EFI_DEVICE_ERROR;
    //DEBUG((EFI_D_INFO,"AtadSmiHandler11\n"));
...
)
{
    ...
    DEBUG((EFI_D_INFO, "AtadSmiSmmSwDispatchCallback \n"));
    ...
    DEBUG((EFI_D_INFO, "AtadSmiSmmSwDispatchCallback2 \n"));
    ...
    Status  = pSwDispatch->Register ( pSwDispatch,
                                      ...
                                      &Handle );
    if (EFI_ERROR(Status)) return Status;
    DEBUG((EFI_D_INFO, "AtadSmiSmmSwDispatchCallback3 \n"));
...
)
{
    ...
    DEBUG((EFI_D_INFO, "In AtadSmi InSmmFunction\n"));
    ...
    if (EFI_ERROR(Status))
    {
    	Status = RegisterProtocolCallback ( &gEfiSmmSwDispatch2ProtocolGuid,
                                        	...
                                        	&EvtAtadSmi,
                                        	&RegAtadSmi );
    	DEBUG((EFI_D_INFO, "In AtadSmi InSmmFunction RegisterProtocolCallback\n"));
