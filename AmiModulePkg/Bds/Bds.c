...
//Z123-004(1) - start
{
    UINT8       BusNum=0,CMOS_0x50=0;
    UINT32      DIDVID=0,u32temp=0;
        
    IoWrite32(0xcf8,(0x80000000+(((0x0F<<3)+0x0)<<8)+0x18));        //Bus:0 Dev:0x0F Fun:0x0
    u32temp = (UINT32)IoRead32(0xcfc);
    BusNum = (UINT8)(u32temp>>8);                                   //Read Secondary Bus Number
  
    IoWrite32(0xcf8,(0x80000000+(((BusNum<<8)+0x0)<<8)+0x00));      //Check 5G PCIe Card present?
    DIDVID = (UINT32)IoRead32(0xcfc);
    IoWrite8(0x70,0x50);                                            //CMOS_5G_4G_FLAG -> CmosBoard.ssp
    CMOS_0x50 = IoRead8(0x71);
    
    if ((DIDVID != 0xFFFFFFFF) && (CMOS_0x50 != 0xF0)){             //Detect 5G module?
        IoWrite8(0x70,0x50);
        IoWrite8(0x71,0x01);                                        //Set Flag
        IoWrite8(0xcf9,0x06);                                       //System Reset
        while(1);    
    }

    IoWrite8(0x70,0x50);
    IoWrite8(0x71,0xFF);                                            //Clear Flag    
}
//Z123-004(1) - end
