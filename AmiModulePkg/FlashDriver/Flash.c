...
#define IsFlashPropertySet(PropertyBit) ((FlashProperties & (PropertyBit)) == (PropertyBit))

static UINTN FlashEmpty = (UINTN)-1;
static UINT32 FlashBlockSize = FLASH_BLOCK_SIZE; //Configuration.sdl
static UINT32 FlashProperties = 0;
...
)
{
    ...
    if (    IsFlashPropertySet(FLASH_PROPERTY_NON_MEMORY_MAPPED)
    	 || IsFlashPropertySet(FLASH_PROPERTY_NON_MEMORY_MAPPED_ON_WRITE_ENABLE) && FlashData.WriteEnableCounter!=0
    )
    {
        BOOLEAN Status;
...
BOOLEAN IsClean(IN UINT8 *Address, IN UINTN Size)
{
    ...
    if (!( (UINTN)Address & (INT_SIZE-1) ))
    {
        ...
        {
            if (IsFlashPropertySet(FLASH_PROPERTY_NON_MEMORY_MAPPED_ON_WRITE_ENABLE))
            {
                UINTN nData=0;	
...
EFI_STATUS FlashDriverCommonInit(){
	EFI_STATUS Status;
	
    Status = CreateCriticalSection(&Cs);
    ASSERT_EFI_ERROR(Status);
    
    FlashProperties = PcdGet32(AmiPcdFlashPropertyMask);
    
    // Backward Compatibility Support.
    // Older code uses SDL tokens to describe flash device properties.
#if NO_MMIO_FLASH_ACCESS_DURING_UPDATE
    FlashProperties |= FLASH_PROPERTY_NON_MEMORY_MAPPED_ON_WRITE_ENABLE;
#endif
#ifdef FLASH_ERASE_POLARITY
    if (FLASH_ERASE_POLARITY==0) FlashProperties |= FLASH_PROPERTY_ZERO_ERASE_POLARITY;
#endif
    
    FlashEmpty =  IsFlashPropertySet(FLASH_PROPERTY_ZERO_ERASE_POLARITY) ? 0 : (UINTN)-1;
