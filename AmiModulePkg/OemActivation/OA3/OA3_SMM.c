...
EFI_STATUS Oa3UpdateAcpiTable( IN UINT8 *ProductKey )
{
    ...
    Status = AmiValidateMemoryBuffer((VOID*)ProductKey, (sizeof(EFI_ACPI_MSDM_TABLE) - sizeof(ACPI_HDR)));
    if (EFI_ERROR(Status))
        return Status;
    ...
    Status = AmiValidateMemoryBuffer((VOID*)MsdmVariable.XsdtAddress, sizeof(ACPI_HDR));
    if (EFI_ERROR(Status)) 
        return Status;
    ...
    if (AcpiHdr->Length < sizeof(ACPI_HDR))
        return EFI_INVALID_PARAMETER;
