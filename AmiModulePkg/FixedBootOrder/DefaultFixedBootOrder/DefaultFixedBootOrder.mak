...
#if CSM_SUPPORT $(EOL)\
#define FBO_LEGACY_DEVICE_TYPE_NUM $(lastword $(FBO_LEGACY_INDEX)) $(EOL)\
#else$(EOL)\
#define FBO_LEGACY_DEVICE_TYPE_NUM 0 $(EOL)\
#endif$(EOL)\
#define FBO_UEFI_DEVICE_TYPE_NUM $(lastword $(FBO_UEFI_INDEX)) $(EOL)\
#define FBO_DUAL_DEVICE_TYPE_NUM $(lastword $(FBO_DUAL_INDEX2)) $(EOL)"\
...
MATCH_UEFI_FORM_LABEL_END_AUTO_ID=MAKEFILE_INSERT_UEFI_FORM_LABEL_END_AUTO_ID_FLAG
MATCH_LEGACY_FORM_LABEL_AUTO_ID=MAKEFILE_INSERT_LEGACY_FORM_LABEL_AUTO_ID_FLAG
MATCH_LEGACY_FORM_LABEL_END_AUTO_ID=MAKEFILE_INSERT_LEGACY_FORM_LABEL_END_AUTO_ID_FLAG
...
FBO_DUAL_INDEX2 := $(wordlist 1,$(words $(FBO_DUAL_DEVICE1)),$(NONE_ZERO_INDEX_ARRAY))
...
## Prepare Uefi form label end auto id ##
$(FBO_UEFI_FORM_LABEL_END_AUTO_ID) : $(BUILD_DIR)/Token.mak 
FBO_UEFI_FORM_LABEL_END_AUTO_ID := $(foreach INDEX, $(FBO_UEFI_DEVICE),\
AUTO_ID($(INDEX)_BOOT_FORM_LABEL_END)$(EOL))
...
## Prepare Legacy form label end auto id ##
$(FBO_LEGACY_FORM_LABEL_END_AUTO_ID) : $(BUILD_DIR)/Token.mak 
FBO_LEGACY_FORM_LABEL_END_AUTO_ID := $(foreach INDEX, $(FBO_LEGACY_DEVICE),\
AUTO_ID($(INDEX)_BOOT_FORM_LABEL_END)$(EOL))
...
ifeq ($(FBO_SUPPORT_ADD_BOOT_OPTION), 1)
ifneq ($(TSE_SOURCES_SUPPORT), 1)
$(error Add/Delete boot option only support when Tse source exists.)
endif
endif
...
$(BUILD_DIR)/DefaultFixedBootOrder.sd : $(BUILD_DIR)/Token.mak  \
	...
	@cat $(DefaultFixedBootOrder_DIR)/DefaultFixedBootOrder.sd \
	    ...
	    | gawk "{sub(/$(MATCH_UEFI_FORM_LABEL_END_AUTO_ID)/, \"$(FBO_UEFI_FORM_LABEL_END_AUTO_ID)\"); print}" \
	    | gawk "{sub(/$(MATCH_LEGACY_FORM_LABEL_AUTO_ID)/, \"$(FBO_LEGACY_FORM_LABEL_AUTO_ID)\"); print}" \
	    | gawk "{sub(/$(MATCH_LEGACY_FORM_LABEL_END_AUTO_ID)/, \"$(FBO_LEGACY_FORM_LABEL_END_AUTO_ID)\"); print}" \
	...
	gawk -f $(DefaultFixedBootOrder_DIR)/AutoGenFboData.gawk \
		-v TYPE=LegacyPriorities -v DEVICE=$(GAWK_LEGACY_DEVICE)\
		$(BUILD_DIR)/DefaultFixedBootOrder.sd
	@cat $(BUILD_DIR)/AutoGenFboData.sd > $(BUILD_DIR)/DefaultFixedBootOrder.sd
	
	gawk -f $(DefaultFixedBootOrder_DIR)/AutoGenFboData.gawk \
		-v TYPE=UefiPriorities -v DEVICE=$(GAWK_UEFI_DEVICE)\
		$(BUILD_DIR)/DefaultFixedBootOrder.sd
	@cat $(BUILD_DIR)/AutoGenFboData.sd > $(BUILD_DIR)/DefaultFixedBootOrder.sd
	
	gawk -f $(DefaultFixedBootOrder_DIR)/AutoGenFboData.gawk \
		-v TYPE=DualPriorities -v DEVICE=$(GAWK_DUAL_DEVICE)\
		$(BUILD_DIR)/DefaultFixedBootOrder.sd
	@cat $(BUILD_DIR)/AutoGenFboData.sd > $(BUILD_DIR)/DefaultFixedBootOrder.sd
	...
	gawk -f $(DefaultFixedBootOrder_DIR)/AutoGenFboData.gawk \
		...
		-v FBO_START_KEY=$(FBO_ITEM_START_KEY)\
		$(BUILD_DIR)/DefaultFixedBootOrder.sd
	@cat $(BUILD_DIR)/AutoGenFboData.sd > $(BUILD_DIR)/DefaultFixedBootOrder.sd
	gawk -f $(DefaultFixedBootOrder_DIR)/AutoGenFboData.gawk \
...
$(BUILD_DIR)/FboGroupForm.vfr :  $(BUILD_DIR)/Token.mak  \
	...
	@cat $(FboGroupForm_DIR)/FboGroupForm.vfr \
