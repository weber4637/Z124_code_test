...
#include <Guid/MemoryAttributesTable.h>    // Denverton AptioV Override  - EIP#419449
...
// Denverton AptioV Override Start - EIP#419449
...
// Denverton AptioV Override End - EIP#419449
     ...
     // Denverton AptioV Override Start - EIP#419449
        if (IS_RT_MEMORY_DESCRIPTOR(Descriptor)){
            ...
            if (Descriptor->Type == EfiReservedMemoryType){
               ...
               if ( GcdDescriptor.GcdMemoryType != EfiGcdMemoryTypeSystemMemory ) continue;
            }
...
EFI_STATUS SmmAmiBufferValidationLibGetMemoryAttributesTable(){
    ...
    Status = EfiGetSystemConfigurationTable(&gEfiMemoryAttributesTableGuid, &AttrTable);
    if (EFI_ERROR(Status)) return Status;
