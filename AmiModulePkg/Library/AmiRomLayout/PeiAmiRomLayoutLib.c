...
EFI_STATUS AmiGetRomLayoutProperties(
	...
	if (RomLayoutSize != NULL) *RomLayoutSize = (AmiRomLayoutHob->Header.Header.HobLength - sizeof(AMI_ROM_LAYOUT_HOB))
...
EFI_STATUS AmiGetImageRomLayout(
    ...
    if (ImageAddress == NULL || ImageSize == 0 || RomLayout == NULL)
        return EFI_INVALID_PARAMETER;

    SearchPointer = (UINT32 *)((UINT8*)ImageAddress - sizeof(AMI_ROM_LAYOUT_SECTION) + ImageSize);
    ASSERT( (UINTN)SearchPointer == ((UINTN)SearchPointer & (~0x03)) );
    ...
    Signature = gAmiRomLayoutFfsFileGuid.Data1;
    for ( ; SearchPointer != (UINT32*)ImageAddress; SearchPointer--)
    {
        if (*SearchPointer != Signature) continue;
...
*/
EFI_STATUS AmiGetRomLayout(
    OUT AMI_ROM_AREA **RomLayout, OUT UINT32 *Version OPTIONAL, 
    OUT UINT32 *RomAreaSize OPTIONAL, OUT UINT32 *RomLayoutSize OPTIONAL
){
    UINT32 AreaSize, TotalSize;
    EFI_STATUS Status;
    AMI_ROM_AREA *RomArea;
    UINT8* TablePtr;
    
    if (RomLayout == NULL) return EFI_INVALID_PARAMETER;
    
    Status = AmiGetRomLayoutProperties(Version, &AreaSize, &TotalSize);
    if (EFI_ERROR(Status)) return Status;
    
    if (RomAreaSize != NULL) *RomAreaSize = AreaSize;
    if (RomLayoutSize != NULL) *RomLayoutSize = TotalSize;
    
    Status = PeiServicesAllocatePool(TotalSize, RomLayout);
    if (EFI_ERROR(Status)) return Status;

    for(  TablePtr = (UINT8*)*RomLayout, RomArea = AmiGetFirstRomArea()
        ; RomArea != NULL
        ; TablePtr += AreaSize, RomArea = AmiGetNextRomArea(RomArea)
    ){
        (*GetPeiServicesTablePointer())->CopyMem(TablePtr, RomArea, AreaSize);
    }
