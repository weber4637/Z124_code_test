...
//-------------------------------------------------------------
	...
	if(PciCommon!=NULL){
		...
		if(EFI_ERROR(Status)){
			...
			PciCommon->BmeDmaMitigation = PCI_BME_DMA_MITIGATION_DEFAULT_VALUE;   // Denverton AptioV Override - EIP#402042
			if(Status==EFI_NOT_FOUND) {
...
//#endif
		...
		if (PciCommon!=NULL){
			...
			PciCommon->BmeDmaMitigation=SetupData->BmeDmaMitigation;   // Denverton AptioV Override - EIP#402042
		}
