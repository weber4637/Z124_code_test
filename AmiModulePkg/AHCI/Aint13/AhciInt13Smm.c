...
)
{
    *ReadValue = *(UINT32*)(AhciMmioAddress);
    return EFI_SUCCESS; // Denverton AptioV Override - EIP#501277
}
...
)
{
    ...
    // Check if the input buffer size is valid.
    if (*CommBufferSize != sizeof(AHCI_INT13_SMM_DATA)) {
        return EFI_SUCCESS;
    }
    // Validate if CommBuffer address is non-SMRAM region to avoid SMRAM data 
    // corruption through this handler.
    Status = AmiValidateMemoryBuffer(CommBuffer, sizeof(AHCI_INT13_SMM_DATA));
    if (EFI_ERROR(Status)) {
        return EFI_SUCCESS;
    }
    // Denverton AptioV Override End - EIP#501277
    ...
    if (AhciInt13SmmData->DriveCount == 0 || AhciInt13SmmData->DriveCount > SATA_PORT_COUNT) { // Denverton AptioV Override - EIP#501277
        return EFI_SUCCESS;
    }
