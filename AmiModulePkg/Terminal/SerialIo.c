...
/**
   @internal
    ...
    @param  EFI_COMPONENT_NAME_PROTOCOL  *This
    @param  CHAR8                        *Language,
    @param  CHAR16                       **DriverName
...
SerialTest:
    ...
    Parameters->SerialMode.ReceiveFifoDepth = 0;
	// If project is in Debug mode and FIFO_ENABLE_64 BIT is set, assign ReceiveFifoDepth as 64-bytes
#ifdef EFI_DEBUG
    if ((PcdGet8(PcdSerialFifoControl) & FIFO_ENABLE_64B) != 0) {
        Parameters->SerialMode.ReceiveFifoDepth = UART_MAX_RECEIVE_FIFO_DEPTH;
    }
#endif
        
    Status = Parameters->SerialIo.SetAttributes(&Parameters->SerialIo,
                    0,              //Parameters->SerialMode.BaudRate
                    Parameters->SerialMode.ReceiveFifoDepth,              //Parameters->SerialMode.ReceiveFifoDepth
...
)
{
    ...
    if ( ReceiveFifoDepth == 0 ) {
		// If UartType is 16750 or higher, assign ReceiveFifoDepth as 64-bytes else use default FifoDepth
        if(Parameters->UartType < UART16750){
...
)
{
	...
	// Write the FCR with enabled FIFO_Enable and FIFO_ENABLE_64 BIT
    SerialWritePort(Parameters, SERIAL_REGISTER_FCR, (TRFIFOE|FIFO_ENABLE_64B));
	//Read the IIR register and check FIFO_ENABLE_64 BIT is set
    IIRValue = SerialReadPort(Parameters, SERIAL_REGISTER_IIR);
    FifoEnableStatus = IIRValue & (FIFOES | FIFO_ENABLE_64B);

	// If FIFO_ENABLE_64 BIT is set, Set UartType as 16750
    if (FifoEnableStatus == (FIFO_ENABLED|FIFO_ENABLE_64B)) {
        Parameters->UartType = UART16750;
    } else if (FifoEnableStatus == FIFO_ENABLED) {
...
{
    if ((Parameters->UartType < UART16550A) || (NewFifoDepth == 1)) {
        SerialWritePort(Parameters, SERIAL_REGISTER_FCR, ~TRFIFOE);
    } else {
		
		// If UartType is 16750 & Fifo_Depth is 64, enable FIFO_ENABLE_64B bit
		if ((Parameters->UartType >= UART16750) && NewFifoDepth == UART_MAX_RECEIVE_FIFO_DEPTH) {
		    SerialWritePort(Parameters, SERIAL_REGISTER_FCR, 
		                                TRFIFOE|RESETRF|RESETTF|FIFO_ENABLE_64B);
		    Parameters->SerialMode.ReceiveFifoDepth = UART_MAX_RECEIVE_FIFO_DEPTH;	// Set ReceiveFifoDepth as 64-byte
        } else {
            SerialWritePort(Parameters, SERIAL_REGISTER_FCR, 
                            TRFIFOE|RESETRF|RESETTF);
            Parameters->SerialMode.ReceiveFifoDepth = 0x10;
        }
    }
