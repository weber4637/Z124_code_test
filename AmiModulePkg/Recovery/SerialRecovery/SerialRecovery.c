...
)
{
    ...
    do {
        ...
        *ReadChar = '\n';
        ...
        DEBUG((DEBUG_INFO, "\n Recovery from Serial Address %x",(UINT64)ComDevice->SerialDeviceBaseAddress));
        ...
        do {
            ...
            if ( *ReadChar=='N' || *ReadChar=='n' ) {
                ...
                //
                message = "Trying next serial device if available.\n\r";   // Denverton AptioV Override - EIP#353139
                SendSerialString(message);
                Recovery = FALSE;
                break; 
            }
        ...
        if ( !ReadFileFromSerial( pBuff, &FileSize, &Result )) {
            ...
            //
            if( ExtendedVerification || FileSize == (UINT32)RecoveryCapsuleSize ) {   // Denverton AptioV Override - EIP#353139
                return EFI_SUCCESS;
		    // Denverton AptioV Override Start - EIP#353139 
            } else {
                // Try next device as filesize doesn't match.
                DEBUG((DEBUG_INFO, "\n FileSize not match,RecoveryCapsuleSize = %d, Received FileSize=%d\r\n", 
                        RecoveryCapsuleSize,FileSize));
                DEBUG((DEBUG_INFO, " Trying next serial device if available.\n\r"));
                Index++;
                continue;
            }
			// Denverton AptioV Override Stop - EIP#353139
        }
    } while( TRUE );
...
)
{
    ...
    NumBlocks++;
    while ( NumBlocks ) {
        Status = FALSE;   // Denverton AptioV Override - EIP#353139
        ...
        if ( Data == XMODEM_SOH ) {
            ...
            // Denverton AptioV Override Start - EIP#353139
            // If data packet is not recevied, Send NACK 
            // to inform Sender to resend the packet
            if ( !Status ) {
                Command = XMODEM_NAK;
            }
            // Denverton AptioV Override End - EIP#353139
...
{
    UINTN    Result;
    UINTN    MaxDelay = 100;// ms   // Denverton AptioV Override - EIP#353139

    // Timeout of 100ms
    while(!SerialPortPoll() && MaxDelay) {   // Denverton AptioV Override Start- EIP#353139
        mStallPpi->Stall( mPeiServices, mStallPpi, 1000 );   // Denverton AptioV Override End- EIP#353139
        MaxDelay--;
    }   // Denverton AptioV Override - EIP#353139
