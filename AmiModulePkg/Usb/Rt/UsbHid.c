...
)
{
    ...
    USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "UsbHidDevType: %x, PollingLength: %x, PollInterval: %x\n", 
        DevInfo->HidDevType, DevInfo->PollingLength, DevInfo->bPollInterval);
    ...
    if ((DevInfo->wVendorId == MICROSOFT_VID) && (DevInfo->wDeviceId == MICROSOFT_KB600_DID)) {
       DEV_INFO*   FirstDevInfo;
       UINTN       DeviceIndex;

       if (DevInfo->IntInEndpoint == (EP_DESC_ADDR_DIR_BIT | EP_DESC_ADDR_EP_NUM_1))  return DevInfo;

       for (DeviceIndex = 1; DeviceIndex < MAX_DEVICES; DeviceIndex++) {
           FirstDevInfo = &gUsbData->aDevInfoTable[DeviceIndex];
           if ((FirstDevInfo->Flag & DEV_INFO_VALIDPRESENT) != DEV_INFO_VALIDPRESENT) {
               continue;
           }
           if (FirstDevInfo->bDeviceAddress == DevInfo->bDeviceAddress) {
               if (FirstDevInfo->IntInEndpoint == (EP_DESC_ADDR_DIR_BIT | EP_DESC_ADDR_EP_NUM_1)) {
                   if ((FirstDevInfo->HidDevType & (HID_DEV_TYPE_KEYBOARD | HID_DEV_TYPE_MOUSE | 
                      ...
                      (FirstDevInfo->PollingLength != 0) && (FirstDevInfo->bPollInterval != 0)) {
                       if (!((FirstDevInfo->HidDevType & HID_DEV_TYPE_MOUSE) && (gUsbData->dUSBStateFlag & USB_FLAG_EFIMS_DIRECT_ACCESS))) {
                           (*gUsbData->aHCDriverTable[GET_HCD_INDEX(HcStruc->bHCType)].pfnHCDActivatePolling)(HcStruc, FirstDevInfo);
                       }
                   }
                   break;
               }
