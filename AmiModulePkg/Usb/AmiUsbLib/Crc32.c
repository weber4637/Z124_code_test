...
extern AMI_USB_SMM_PROTOCOL    *gAmiUsbSmmProtocol; 
  ...
  )
{
  ...
  UINT32   RandomValue =  gAmiUsbSmmProtocol->GlobalDataValidation.RandomValue; 
  
  //
  // Create mCrcTable[256] randomly, the random value was created when installing  gAmiUsbSmmProtocol.
  //
  for (TableEntry = 0; TableEntry < 256; TableEntry++) {
    Value = ReverseBits ((UINT32) TableEntry);
    for (Index = 0; Index < 8; Index++) {
      if ((Value & 0x80000000) != 0) {
        Value = (Value << 1) ^ RandomValue; 
      } else {
        Value = Value << 1;
      }
    }
