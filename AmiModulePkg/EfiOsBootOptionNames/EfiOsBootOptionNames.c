...
CHAR16* CreateNameWithUefiOS( IN EFI_DEVICE_PATH_PROTOCOL *DevicePath, IN CHAR16 *BootOptionName )
{
    ...
    if (FullDevicePath != NULL)
    {
        ...
        if (EFI_ERROR(Status))
        {
            DEBUG((DEBUG_VERBOSE | DEBUG_ERROR, "CreateNameWithUefiOS :: Does not support Simple File System.\n"));
            ...
            if (EFI_ERROR(Status))
            {
                DEBUG((DEBUG_VERBOSE | DEBUG_ERROR, "CreateNameWithUefiOS :: AllocatePool: %r\n", Status));
                return NULL;
            }
        ...
        else
        {
            ...
            if (EFI_ERROR(Status))
            {
                DEBUG((DEBUG_VERBOSE | DEBUG_ERROR, "CreateNameWithUefiOS :: LocateDevicePath: %r\n", Status));
                return NULL;
            }
