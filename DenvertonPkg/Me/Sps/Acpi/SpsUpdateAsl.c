  ...
  IN EFI_SYSTEM_TABLE           *pSystemTable)
{
  ...
  {
    DEBUG((DEBUG_ERROR, "[SPS] ERROR: Connot locate SPS AML firmware volume\n"));
       ...
       Instance++, pAcpiHeader = NULL)
  {
    ...
    switch (pAcpiHeader->OemTableId)
    {
      case SPS_ASL_SIG_NM:
        //
        // In this table need to update 'H2CR' region address with the actuall HECI-2 MBAR address.
             ...
             pCurr < (UINT8*)pAcpiHeader + pAcpiHeader->Length; pCurr++)
        {
          ...
          switch (Signature)
          {
            case AML_SIGNATURE_32('H', '1', 'S', 'T'):
              //
              // HECI-1 status for _STA. Chcek if it is declaration, not a reference.
              //
            ...
            case AML_SIGNATURE_32('H', '2', 'S', 'T'):
              //
              // HECI-2 status for _STA. Chcek if it is declaration, not a reference.
              //
              if (pCurr[4] == AML_BYTE_PREFIX)
              {
            ...
            case AML_SIGNATURE_32('H', '1', 'C', 'S'):
              //
              // HECI-1 CSR region. Chcek if it is declaration, not a reference.
              //
            ...
            case AML_SIGNATURE_32('H', '2', 'C', 'S'):
              //
              // HECI-2 CSR region. Chcek if it is declaration, not a reference.
              //
              if (pCurr[4] == AML_REGTYPE_SYSMEM &&  pCurr[5] == AML_QWORD_PREFIX)
              {
            ...
            case AML_SIGNATURE_32('T', 'H', 'N', 'U'):
              //
              // Processor threads number. Chcek if it is declaration, not a reference.
              //
    ...
    if (PcdGetBool(PcdUpdateSpsAcpiOemTableId))
    {
      ...
      CopyMem (&pAcpiHeader->OemId, &TempOemId, 6);
      CopyMem (&pAcpiHeader->OemTableId, &TempOemTableId, 8);
