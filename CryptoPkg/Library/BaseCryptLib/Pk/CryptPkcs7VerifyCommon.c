  ...
  )
{
  ...
  if (Wrapped) {
    ...
    *WrapData     = local_malloc (*WrapDataSize);  //APTIOV OVERRIDE - To avoid redefinition build error 
    if (*WrapData == NULL) {
      *WrapFlag = Wrapped;
      return FALSE;
    }
  ...
  )
{
  ...
  Buffer = local_malloc (Length);  //APTIOV OVERRIDE - To avoid redefinition build error 
  if (Buffer == NULL) {
    goto _Exit;
  }
...
_Exit:
  ...
  if (!Status && (Buffer != NULL)) {
    local_free (Buffer);  //APTIOV OVERRIDE - To avoid redefinition build error 
  }
  ...
  )
{
  ...
  for (Index = 0; ; Index++) {
    ...
    CertBuf    = local_malloc (BufferSize);  //APTIOV OVERRIDE - To avoid redefinition build error 
    ...
    if (OldBuf != NULL) {
      ...
      local_free (OldBuf);  //APTIOV OVERRIDE - To avoid redefinition build error 
      OldBuf = NULL;
    }
    ...
    local_free (SingleCert);  //APTIOV OVERRIDE - To avoid redefinition build error 
    SingleCert = NULL;
  }
  ...
  if (CertBuf != NULL) {
    ...
    *TrustedCert = local_malloc (*CertLength);  //APTIOV OVERRIDE - To avoid redefinition build error 
    if (*TrustedCert == NULL) {
      goto _Exit;
    }
...
_Exit:
  ...
  //
  if (!Wrapped) {
    local_free (SignedData);  //APTIOV OVERRIDE - To avoid redefinition build error 
  }
  ...
  if (!Status && (CertBuf != NULL)) {
    local_free (CertBuf);  //APTIOV OVERRIDE - To avoid redefinition build error 
    *CertStack = NULL;
  }
  ...
  )
{
  ...
  local_free (Certs);   //APTIOV OVERRIDE - To avoid redefinition build error 
}
...
_Error:
  ...
  if (!Wrapped && (NewP7Data != NULL)) {
    local_free (NewP7Data);  //APTIOV OVERRIDE - To avoid redefinition build error 
  }
