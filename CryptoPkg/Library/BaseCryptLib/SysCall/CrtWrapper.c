...
//APTIOV OVERRIDE starts- Controlling build of wrapper methods based on SDL token
#if defined DISABLE_STDLIB_WRAPPER_METHODS && DISABLE_STDLIB_WRAPPER_METHODS==0

int errno = 0;

/* APTIOV OVERRIDE - When AmiModulePkg >= 39 is used, IntrinsicsLib from AmiModulePkg will be consumed 
 in IA32 instead of CryptoPkg's IntrinsicLib. So adding "_fltused" here to avoid build error.*/
int _fltused  = 1;
#endif  //APTIOV OVERRIDE ends- Controlling build of wrapper methods based on SDL token
...
void qsort (void *base, size_t num, size_t width, int (*compare)(const void *, const void *))
{
  ...
  //
  Buffer = local_malloc (width); //APTIOV OVERRIDE - To avoid redefinition build error 
                                 //as malloc() function is alreday defined in StdLib
  ASSERT (Buffer != NULL);
  ...
  local_free (Buffer); //APTIOV OVERRIDE - To avoid redefinition build error 
                       //as free() function is already defined in StdLib
  return;
}
