...
//  INTN *timer
//  )
time_t local_time (time_t *timer) //APTIOV OVERRIDE - To avoid redefinition build error 
                                  //as time() function is defined in StdLib
{
  EFI_STATUS  Status;
  EFI_TIME    Time;
  time_t      CalTime;
...
//
struct tm * local_gmtime (const time_t *timer) //APTIOV OVERRIDE - To avoid redefinition build error 
                                               //as gmtime() function is defined in StdLib
{
  struct tm  *GmTime;
  UINT16     DayNo;
  UINT16     DayRemainder;
  ...
  GmTime = local_malloc (sizeof (struct tm)); //APTIOV OVERRIDE - To avoid redefinition build error 
