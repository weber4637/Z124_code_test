...
void *local_malloc (size_t size) //APTIOV OVERRIDE - To avoid redefinition build error 
                                 //as malloc() function is defined in StdLib
{
  return RuntimeAllocateMem ((UINTN) size);
}

/* Reallocate memory blocks */
void *local_realloc (void *ptr, size_t size) //APTIOV OVERRIDE - To avoid redefinition build error 
                                             //as realloc() function is defined in StdLib
{
  VOID   *NewPtr;
  UINTN  StartOffset;
  UINTN  StartPageIndex;
  ...
  if (ptr == NULL) {
    return local_malloc (size); //APTIOV OVERRIDE - To avoid redefinition build error 
	                            //as malloc() function is defined in StdLib
  }
...
void local_free (void *ptr) //APTIOV OVERRIDE - To avoid redefinition build error 
                            //as free() function is defined in StdLib
{
  //
