                          ...
                          const unsigned char *pers, size_t perslen)
{
    ...
    drbg->reseed_time = local_time(NULL);  //APTIOV OVERRIDE - To avoid re-definition build error
                       ...
                       const unsigned char *adin, size_t adinlen)
{
    ...
    if (drbg->reseed_time_interval > 0) {
        time_t now = local_time(NULL);  //APTIOV OVERRIDE - To avoid re-definition build error
	                                    //as time() is defined in Stdlib.
