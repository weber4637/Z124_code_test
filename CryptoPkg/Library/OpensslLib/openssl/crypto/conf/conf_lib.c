...
{
    OPENSSL_INIT_SETTINGS *ret = local_malloc(sizeof(*ret));	//APTIOV OVERRIDE - To avoid re-definition build error
	                                                            //as malloc() is defined in Stdlib.
                                     ...
                                     const char *filename)
{
    ...
    local_free(settings->filename);  //APTIOV OVERRIDE - To avoid re-definition build error
                                    ...
                                    const char *appname)
{
    ...
    local_free(settings->appname);	//APTIOV OVERRIDE - To avoid re-definition build error
	                                //as free() is defined in Stdlib.
...
{
//APTIOV OVERRIDE starts - To avoid re-definition build error as free() is defined in Stdlib.
    local_free(settings->filename);
    local_free(settings->appname);
    local_free(settings);		
//APTIOV OVERRIDE ends - To avoid re-definition build error as free() is defined in Stdlib.	
}
