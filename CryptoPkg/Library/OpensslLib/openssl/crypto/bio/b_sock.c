...
int BIO_accept(int sock, char **ip_port)
{
    ...
    if (ip_port != NULL) {
        ...
        if (host != NULL && port != NULL) {
            //APTIOV Override to avoid buffer overlap assert.
            maxlen = strlen(host) + strlen(port) + 2;
            *ip_port = OPENSSL_zalloc(maxlen);
            //APTIOV Override to avoid buffer overlap assert.
        }
        else
            *ip_port = NULL;
        ...
        if (*ip_port == NULL) {
            ...
            //APTIOV Override to avoid buffer overlap assert.
            AsciiStrCpyS(*ip_port, maxlen, host);
            AsciiStrCatS(*ip_port, maxlen, ":");
            AsciiStrCatS(*ip_port, maxlen, port);
            //APTIOV Override to avoid buffer overlap assert.
        }
        OPENSSL_free(host);
        OPENSSL_free(port);
