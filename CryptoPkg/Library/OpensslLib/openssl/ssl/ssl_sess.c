...
SSL_SESSION *SSL_SESSION_new(void)
{
    ...
    ss->time = (unsigned long)local_time(NULL);  //APTIOV OVERRIDE - To avoid re-definition build error
...
int ssl_get_prev_session(SSL *s, CLIENTHELLO_MSG *hello)
{
    ...
    if (ret->timeout < (long)(local_time(NULL) - ret->time)) { /* timeout */   //APTIOV OVERRIDE - To avoid re-definition build error
	                                                                           //as time() is defined in Stdlib.
