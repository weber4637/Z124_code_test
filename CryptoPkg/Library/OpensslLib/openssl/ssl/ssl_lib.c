...
void ssl_update_cache(SSL *s, int mode)
{
    ...
    if ((!(i & SSL_SESS_CACHE_NO_AUTO_CLEAR)) && ((i & mode) == mode)) {
        ...
        if ((tsan_load(stat) & 0xff) == 0xff)
            SSL_CTX_flush_sessions(s->session_ctx, (unsigned long)local_time(NULL)); //APTIOV OVERRIDE - To avoid re-definition build error
                          ...
                          size_t parameter_2_len)
{
    ...
    AsciiStrCpyS(cursor, out_len, prefix);  //APTIOV Override to avoid buffer overlap assert.
    cursor += prefix_len;
    *cursor++ = ' ';
