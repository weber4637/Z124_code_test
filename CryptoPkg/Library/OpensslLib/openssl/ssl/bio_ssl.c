...
static int ssl_read(BIO *b, char *buf, size_t size, size_t *readbytes)
{
    ...
    case SSL_ERROR_NONE:
        ...
        if ((sb->renegotiate_timeout > 0) && (!r)) {
            ...
            tm = (unsigned long)local_time(NULL);   //APTIOV OVERRIDE - To avoid re-definition build error
...
static long ssl_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    case BIO_C_SET_SSL_RENEGOTIATE_TIMEOUT:
        ...
        bs->last_time = (unsigned long)local_time(NULL);   //APTIOV OVERRIDE - To avoid re-definition build error
	                                                       //as time() is defined in Stdlib.
        break;
