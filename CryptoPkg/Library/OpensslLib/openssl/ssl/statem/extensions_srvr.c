                          ...
                          size_t chainidx)
{
    ...
    now = (unsigned long)local_time(NULL);   //APTIOV OVERRIDE - To avoid re-definition build error
	                                         //as time() is defined in Stdlib.
                       ...
                       size_t chainidx)
{
    ...
    for (id = 0; PACKET_remaining(&identities) != 0; id++) {
        ...
        if (sess != NULL) {
            ...
            now = (uint32_t)local_time(NULL);   //APTIOV OVERRIDE - To avoid re-definition build error
	                                            //as time() is defined in Stdlib.
                                     ...
                                     X509 *x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_cookie)
            ...
            || !WPACKET_put_bytes_u32(pkt, (unsigned int)local_time(NULL))  //APTIOV OVERRIDE - To avoid re-definition build error
	                                                                        //as time() is defined in Stdlib.
            || !WPACKET_start_sub_packet_u16(pkt)
