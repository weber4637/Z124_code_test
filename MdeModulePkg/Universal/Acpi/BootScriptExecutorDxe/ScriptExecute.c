...
//*** AMI PORTING BEGIN ***//	
  ...
  //Instead of CpuDeadLoop we will reset system in S3Resume.c, based on error Status.
...
//*** AMI PORTING END ***//
  ...
  //*** AMI PORTING BEGIN ***//	    
  if (EFI_ERROR (Status)) {
  //BootScript Executor returned an error. We can't perform system reset here, so just hanging.	  
	 DEBUG ((EFI_D_ERROR, "BootScript Executor returned an error! We can't perform system reset here, so just hanging.\n")); 
     CpuDeadLoop ();
     return Status;
