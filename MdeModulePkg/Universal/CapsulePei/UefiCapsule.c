  ...
  )
{
  ...
  while (Hob.Raw != NULL) {
    ...
    if (ResourceDescriptor->ResourceType == EFI_RESOURCE_SYSTEM_MEMORY) {
      ...
      //*** AMI PORTING BEGIN ***//
            // Improvement: Find highest memory region below 4GB.  The SMM memory should be directly above that
            if (MemoryResource[Index].PhysicalStart > Addr  && MemoryResource[Index].PhysicalStart < 0x100000000)
            {
                Addr = MemoryResource[Index].PhysicalStart;
                SaveIndex = Index;
            }
      //*** AMI PORTING END *****//
      Index++;
  ...
  //*** AMI PORTING BEGIN ***//
  // Improvement: support capsule and/or capsule mailbox in the SMM memory.  This rounds up to include SMM memory
    Addr = MemoryResource[SaveIndex].ResourceLength + MemoryResource[SaveIndex].PhysicalStart;
    
    Addr= ALIGN_VALUE(Addr,0x40000000 );
    
    MemoryResource[SaveIndex].ResourceLength = Addr - MemoryResource[SaveIndex].PhysicalStart;

  DEBUG ((EFI_D_INFO, "\n\n\nMemoryResource[0x%x] - Start(0x%0lx) Length(0x%0lx)\n\n\n",
                SaveIndex, MemoryResource[SaveIndex].PhysicalStart, MemoryResource[SaveIndex].ResourceLength));
  // AMI PORTING END
  return MemoryResource;
}
