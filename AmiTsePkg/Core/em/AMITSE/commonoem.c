...
//G160-001(12) - start
		...
		text = HiiGetString( gHiiHandle, STRING_TOKEN(STR_F7_BBS_POPUP_MENUE) );
	        if ( text != NULL )
			PostManagerDisplayPostMessage(text);
		MemFreePointer( (VOID **)&text );
#endif
...
VOID ProcessEnterSetup(VOID)
{
    ...
    if ( gPasswordType == AMI_PASSWORD_NONE )
    {
        ...
        else
        {
            ...
            if (!EFI_ERROR (Status))
      	    {
//G160-005(18)      	    		PasswordType = gEsaInterfaceForTSE->CheckSystemPassword (AMI_PASSWORD_USER, &NoOfRetries, NULL);
                        PasswordType = gEsaInterfaceForTSE->CheckSystemPassword (AMI_PASSWORD_ADMIN, &NoOfRetries, NULL);       //G160-005(18)
					gPasswordType = PasswordType; // setting the gPasswordType in Esa Boot Only.
      	    }
      	    else
      	    {
//G160-005(18)      	    	PasswordType = CheckSystemPassword (AMI_PASSWORD_USER, &NoOfRetries, NULL);
                        PasswordType = CheckSystemPassword (AMI_PASSWORD_ADMIN, &NoOfRetries, NULL);    //G160-002(20)
      	    }
//G160-005(18)            if(AMI_PASSWORD_NONE == CheckSystemPassword( AMI_PASSWORD_NONE, &NoOfRetries, NULL))        //G160-002(20)      if(AMI_PASSWORD_NONE == PasswordType)
                if(AMI_PASSWORD_NONE == PasswordType)           //G160-005(18)
            {
