...
#define PlatformLang            L"PlatformLang"
...
EFI_STATUS EFIAPI SetupEntry(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    {   //If LastLangCodes Variable is not found, this is first boot after FW upgrade.
        ...
        // Submit resources to HII and set LastLangCodes
    ...
    // Check PlatformLang variable and if it does not exist, use the first lang in PlatformLangCodes.  If it is blank use DEFAULT_LANGUAGE value
    if(!DoesVariableExist(PlatformLang, &gEfiGlobalVariableGuid))
    {
        CHAR8 *String;
        UINTN Size = 0;
        
        // Use the first entry in the LangCodesBuffer
        // Check for NULL and for an empty string.
        if ((LangCodesBuffer != NULL) && (*LangCodesBuffer != '\0'))
        {
            // find the length of the first language code.
            while (LangCodesBuffer[Size] != ';' && LangCodesBuffer[Size] != '\0')
            {
                Size++;
            }
            
            // Allocate the buffer
            gBS->AllocatePool( EfiBootServicesData, Size + 1, (VOID**)&String);
            gBS->CopyMem(String, LangCodesBuffer, Size);
            String[Size] = 0; // terminate string
        }
        else
        {
            Size = sizeof(CONVERT_TO_STRING(DEFAULT_LANGUAGE_CODE));
            gBS->AllocatePool( EfiBootServicesData, Size, (VOID**)&String);
            gBS->CopyMem(String, CONVERT_TO_STRING(DEFAULT_LANGUAGE_CODE), Size);
        }
        gRT->SetVariable(PlatformLang, &gEfiGlobalVariableGuid, 
