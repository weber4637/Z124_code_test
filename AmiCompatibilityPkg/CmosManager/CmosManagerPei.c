...
//G160-029(3) - start
    #if defined(NEXCOM_RTC_WORKAROUND) && (NEXCOM_RTC_WORKAROUND == 1)
    {
        ...
        if(u8temp & 0x04){
                u8temp &= 0xFB;
                IoWrite8(0x70,0x0B);
                IoWrite8(0x71,u8temp);
        }
        
        Address = PCIEX_BASE_ADDRESS + (UINT32)(31 << 15) + (UINT32)(2 << 12) + 0xA4;    //Read Bus:0 Dev:31 Fun:2 offset:0xA4 PMCON_B reg.
        Data = (*(volatile UINT16*)(UINTN)(Address));
        if ((Data & BIT2) != 0) {               //Check RTC_PWR_STS bit?
                Time.Second = 0;
                Time.Minute = 0;
                Time.Hour   = 0;
                Time.Day    = 1; //RTC_INIT_DAY;
                Time.Month  = 1; //RTC_INIT_MONTH;
                Time.Year   = DEFAULT_YEAR; //RTC_INIT_YEAR;
                Time.Nanosecond  = 0;
                Time.TimeZone = EFI_UNSPECIFIED_TIMEZONE;
                Time.Daylight = 0;
                
                Status = Mgr->Access.SetTime(&Mgr->Access,&Time);
         }
    }
    #endif //defined(NEXCOM_RTC_WORKAROUND) && (NEXCOM_RTC_WORKAROUND == 1)
//G160-029(3) - end
